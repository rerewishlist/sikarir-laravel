        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Edit Data Kategori "{{ $item->nama }}"</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formEditKategori{{ $item->id }}" class="row g-3"
                        action="{{ route('datakategori.update', $item->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="col-md-12">
